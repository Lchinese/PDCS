...
# 7. 验证部署结果
echo "[7/8] 验证部署结果..."
kubectl get pods | grep -E 'cart-service|user-service|product-service|web-server|redis|mysql|mongodb'
echo ""

# 8. 检查健康状态
echo "[8/8] 检查服务健康状态..."
RUNNING_COUNT=$(kubectl get pods | grep -E 'cart-service|user-service|product-service' | grep '1/1.*Running' | wc -l)
